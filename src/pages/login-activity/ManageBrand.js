...
import {changeBrand, deleteBrand, getBrand} from "../../api/brandAPI";
...
// const brands = [
//     {brandName : 'Test', organizationRole: 'Owner', verifiedStatus: false, description: 'The best'},
//     {brandName : 'Test1', organizationRole: 'Distributor', verifiedStatus: true, description: 'The best'},
//     {brandName : 'Test2', organizationRole: 'Issuer', verifiedStatus: false, description: 'The best'},
// ]
...
const ManageBrand = () => {
    ...
    useEffect(() => {
        getBrand(id).then(response => {
            setValue("brandName", response.name)
            setValue("role", response.organization_role)
    ...
    const handleChangesSubmit = ({brandName, role, description}) => {
        ...
        changeBrand(id, formData).then(() => {
            setCheckError(true)
            setErrorMessage("Information was changed")
        }).catch(() => {
            setCheckError(true)
    ...
    const handleClickDelete = () => {
      deleteBrand(id).then(() => {
          setCheckError(true)
          setErrorMessage("Brand was deleted")
          navigate(-1)
      }).catch(() => {
          setCheckError(true)
          setErrorMessage("There are some troubles. Try later")
        ...
        <>
            {init && (
                <Box sx={{p: '20px'}} style={{textAlign: 'center', position: 'relative'}}>
                    ...
                    <form noValidate
                          onSubmit={handleSubmit(handleChangesSubmit)}
                    ...
                    >
                        ...
                        <Button className={classes.deleteBtn} size={"large"} onClick={handleClickDelete}>
                            DELETE
                        </Button>
                    </form>
                    <AlertMessage errorMessage={errorMessage} visible={checkError} type={true} close={() => setCheckError(false)}/>
                </Box>
            )}
        </>
    );
